...
| `redis` | standalone, Cluster, Sentinel Redis 자료형·운영 상태 읽기 전용 조회 |
...
각 서버는 read-only 진단·워크플로를 안내하는 대표 MCP prompt도 제공합니다(예: `postgres`의 `diagnose_table`, `gitlab`의 `prepare_mr_review`). 전체 목록은 [사용 가이드](docs/usage.md#mcp-prompts)를 참고하세요.
...
node packages/cli/dist/index.js stdio redis # Redis MCP를 로컬 stdio 방식으로 실행
node packages/cli/dist/index.js stdio docker # Docker MCP를 로컬 stdio 방식으로 실행
